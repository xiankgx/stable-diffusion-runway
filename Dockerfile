...
WORKDIR /
ENV FORCE_CUDA=1 MAX_JOBS=3 TORCH_CUDA_ARCH_LIST="6.0;6.1;6.2;7.0;7.2;7.5;8.0;8.6" python_abi=cp37-cp37m
...
RUN --mount=type=bind,target=whls,from=builder apt-get update && apt-get install -y git ffmpeg libsm6 libxext6 && \
